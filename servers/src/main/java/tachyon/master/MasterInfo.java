    ...
    */
    ...
    for (Map.Entry<String, Double> file : filesBenefit.entrySet()) {
      ...
      for (ClientFileInfo partitionFile: getFilesInfo(new TachyonURI(file.getKey()))) {
        ...
        Pair<String, Double> benefitInfo = new Pair<String, Double>(fileName, benefit);
        for (long blockId : partitionFile.getBlockIds()) {
          partitions.put(blockId, benefitInfo);
        }
      }
    }
        ...
        */
        ...
        for (Long blockId : blocksOnWorker) {
          if (partitions.get(blockId) != null) {
            Pair<String, Double> benefitInfo = partitions.get(blockId);
            if(benefitInfosOnWorker.get(benefitInfo) == null){
              benefitInfosOnWorker.put(benefitInfo, new ArrayList<Long>());
            }
            benefitInfosOnWorker.get(benefitInfo).add(blockId);
          }
        }

        Set<PartitionInfo> partitionsOnWorker = new HashSet<PartitionInfo>();
        for (Map.Entry<Pair<String, Double>, List<Long>> entry : benefitInfosOnWorker.entrySet()) {
          Pair<String, Double> idBenefit = entry.getKey();
          String[] fileName = idBenefit.getFirst().split("_");
          int len = fileName.length;

          partitionsOnWorker.add(new PartitionInfo(Integer.parseInt(fileName[len-2]),
                  Integer.parseInt(fileName[len-1]), idBenefit.getSecond(), entry.getValue()));
        }
    ...
    //then transfer the benefit information to workers
  }
  ...
  public ClientBlockInfo getClientBlockInfo(long blockId) throws FileDoesNotExistException,
    ...
    synchronized (mRootLock) {
      ...
      if (inode == null || inode.isDirectory()) {
        //zengdan add blockId
        throw new FileDoesNotExistException("FileId " + fileId + " for blockId " + blockId + " does not exist.");
      }
      ClientBlockInfo ret =
          ((InodeFile) inode)
