...
public class MasterServiceHandler implements MasterService.Iface {
  ...
  //zengdan
  /*
  public Set<Long> getBlockIDs(long workerId, String rootPath){

    MasterWorkerInfo tWorkerInfo = mWorkers.get(workerId);
    Set<Long> blocks = tWorkerInfo.getBlocks();
    for(Long blockId : blocks) {
      int fileId = BlockInfo.computeInodeId(blockId);
      Inode inode = mFileIdToInodes.get(fileId);
      Inode parent = mFileIdToInodes.get(inode.getParentId());
      parent.getName()   //operatorId
      inode.getName()
      InodeFile file = (InodeFile)inode;
      file.getBlockList().get(0).isInMemory()
    }
  }
  */
  ...
  //zengdan
  @Override
  public void qgmaster_setBenefit(Map<String, Double> filesBenefit)
          throws FileDoesNotExistException, InvalidPathException, TException {
    //mMasterInfo.setWorkerToBenefit(filesBenefit);
  }
  ...
  public void qgmaster_updateBenefit(Map<String, BenefitInfo> filesBenefit)
          throws FileDoesNotExistException, InvalidPathException, TException {
    mMasterInfo.updateWorkerToPartition(filesBenefit);
  }
