...
import tachyon.worker.block.meta.*;
...
public class TieredBlockStore implements BlockStore {
  ...
  //zengdan
  @Override
  public List<TempBlockMeta> createPartitionMeta(long userId, PartitionInfo partition,
      List<BlockStoreLocation> locations) throws IOException {
    mEvictionLock.writeLock().lock();
    try {
      return createPartitionMetaNoLock(userId, partition, locations);
    } finally {
      mEvictionLock.writeLock().unlock();
    }
  }
  ...
  //zengdan
  @Override
  public void movePartition(long userId, PartitionInfo partition, BlockStoreLocation newLocation)
          throws IOException {
    mEvictionLock.writeLock().lock();
    try {
      movePartitionNoLock(userId, partition, newLocation);
    } finally {
      mEvictionLock.writeLock().unlock();
    }
  }

  private void movePartitionNoLock(long userId, PartitionInfo partition,
                                   BlockStoreLocation newLocation) throws  IOException {
    for (long blockId : partition.getBlockIds()) {
      if (!mMetaManager.hasBlockMeta(blockId)) {
        throw new IOException("Failed to move partition: blockId " + blockId + " not exists");
      }
    }
  ...
  //zengdan
  private List<TempBlockMeta> createPartitionMetaNoLock(long userId, PartitionInfo partition,
      List<BlockStoreLocation> locations) throws IOException {
    List<Long> blocks = partition.getBlockIds();
    LOG.info("Create Partition Meta for partition " + partition);
    Long[] blockIds = blocks.toArray(new Long[blocks.size()]);
    List<TempBlockMeta> tempBlocks = new ArrayList<TempBlockMeta>();
    ...
    //partition.setTierLevel(dirView.getParentTierView().getTierViewLevel());
    //partition.setDirIndex(dirView.getDirViewIndex());
  ...
  private void freeSpaceInternal(long userId, BenefitInfo benefit, long initialSize,
    ...
    LOG.info(String.format("Free space with %d blocks evicted and %d blocks tranferred.",
            plan.toEvict().size(), plan.toMove().size()));
  ...
  //zengdan
  private void movePartitionInternal(long userId, PartitionInfo partition, BlockStoreLocation newLocation) throws IOException{
    ...
    // Absent plan means failed to evict enough space.
    if (null == plan) {
      LOG.warn("Failed to move partition: no eviction plan by evictor");
      return;
    }

    LOG.info(String.format("Move partition %s with %d blocks evicted and %d blocks tranferred.",
            partition, plan.toEvict().size(), plan.toMove().size()));
    ...
    for (long blockId : plan.toEvict()) {
      ...
      try {
        ...
        throw new IOException("Failed to move partition: cannot evict block " + blockId);
      } finally {
        mLockManager.unlockBlock(lockId);
      }
    }
    ...
    for (int alias : destTierAlias) {
      ...
      for (Pair<Long, BlockStoreLocation> entry : toMove) {
        long blockId = entry.getFirst();
        BlockStoreLocation newLoc = entry.getSecond();
        BlockMeta blockMeta = mMetaManager.getBlockMeta(blockId);
        BlockStoreLocation oldLocation = blockMeta.getBlockLocation();
        ...
        try {
          ...
          synchronized (mBlockStoreEventListeners) {
            for (BlockStoreEventListener listener : mBlockStoreEventListeners) {
              listener.onMoveBlockByWorker(userId, blockId, oldLocation, newLoc);
            }
          }
        } catch (IOException e) {
          throw new IOException("Failed to move partition: cannot move block " + blockId + " to "
                  + newLoc);
        } finally {
          mLockManager.unlockBlock(lockId);
        }
    ...
    StorageDirView dirView = mAllocator.allocatePartitionWithView(totalSize, Lists.newArrayList(newLocation));
    if (null == dirView) {
      LOG.warn("Failed to move partition: no enough space even after eviction for partition " + partition);
      return;
    }

    LOG.info("Move Partition {} to StorageDirView(tierAlias:{}, dirIndex:{})", partition,
            dirView.getParentTierView().getTierViewAlias(), dirView.getDirViewIndex());

    BlockStoreLocation newLoc = dirView.toBlockStoreLocation();
    for (long blockId : partition.getBlockIds()) {
      BlockMeta blockMeta = mMetaManager.getBlockMeta(blockId);
      BlockStoreLocation oldLocation = blockMeta.getBlockLocation();
      if (oldLocation.belongTo(newLoc)) {
