...
public class BlockServiceHandler implements WorkerService.Iface {
  ...
  //zengdan
  public boolean movePartitionToMemory(long userId, PartitionInfo partition) throws TException {
    try {
      mWorker.movePartition(Users.MIGRATE_DATA_USER_ID, partition, StorageLevelAlias.MEM.getValue());
      return true;
    } catch (IOException ioe) {
      throw new TException(ioe);
    }
  ...
  //zengdan
  public List<String> requestPartitionLocation(long userId, PartitionInfo partition)
          throws OutOfSpaceException {
    try {
      // NOTE: right now, we ask allocator to allocate new blocks in MEM tier
