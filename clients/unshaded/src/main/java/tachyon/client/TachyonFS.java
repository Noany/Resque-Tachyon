...
public class TachyonFS extends AbstractTachyonFS {
  ...
  //zengdan
  public synchronized List<String> getLocalPartitionTemporaryPath(PartitionInfo partition)
          throws IOException {
    List<String> blockPath = mWorkerClient.requestPartitionLocation(partition);
    // TODO: Handle this in the worker?
    for (String path : blockPath) {
      CommonUtils.createBlockPath(path);
    }
    return blockPath;
  }
