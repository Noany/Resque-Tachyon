...
public class WorkerClient implements Closeable {
  ...
  //zengdan
  public synchronized List<String> requestPartitionLocation(PartitionInfo partition)
          throws IOException {
    mustConnect();
  ...
  //zengdan
  public synchronized boolean movePartitionToMemory(PartitionInfo partition)
          throws IOException {
    mustConnect();

    try {
      return mClient.movePartitionToMemory(mMasterClient.getUserId(), partition);
      //return mClient.requestPartitionLocation2(mMasterClient.getUserId(), partition.getId(), partition.getBenefit(), partition.getBlockSize());
    } catch (TException e) {
      mConnected = false;
      throw new IOException(e);
